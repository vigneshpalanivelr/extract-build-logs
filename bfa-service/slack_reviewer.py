...
def slack_events():
    try:
        ...
        if event.get("type") == "message" and not event.get("bot_id"):
            ...
            if not error_text:
                client.chat_postMessage(channel=channel_id, text="⚠️ Could not find the original error context.", thread_ts=thread_ts)
            ...
            try:
                ...
                store_fix(error_id, error_text, new_fix_text, source="edited", approver=display_name, message_ts=message_ts, channel_id=channel_id)
                ...
                client.chat_postMessage(channel=channel_id, text=f"❌ Failed to save your edited fix: {e}", thread_ts=thread_ts)
...
def slack_actions():
    try:
        ...
        if not error_text:
            client.chat_postMessage(channel=channel_id, text="⚠️ Original error text not found. Cannot process this action.")
            return jsonify({"status": "missing_error"}), 200
