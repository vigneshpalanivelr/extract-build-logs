...
class JenkinsLogFetcher:
    ...
    def fetch_stage_log_tail(self, job_name: str, build_number: int, stage_id: str,
        ...
        """
        # Determine tail_lines with proper fallback handling
        if tail_lines is None:
            if self.config and self.config.tail_log_lines:
                tail_lines = self.config.tail_log_lines
            else:
                tail_lines = int(os.getenv('TAIL_LOG_LINES', '5000'))

        # Safety check: ensure tail_lines is valid (should always be set after above logic)
        if not tail_lines or tail_lines <= 0:
